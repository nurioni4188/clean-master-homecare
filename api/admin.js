...
function checkToken(req) {
  const saved = normalizeToken(process.env.ADMIN_TOKEN);
  const body = getBody(req);
  const given = normalizeToken(body.adminToken);
...
export default async function handler(req, res) {
  try {
    ...
    if (action === "list") {
      const r = await fetch(
        `${url}/rest/v1/consultations?select=*&order=created_at.desc&limit=300`,
        {
          method: "GET",
          headers,
        }
